...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option
                .setName('duration')
                .setDescription('Ban duration (e.g., 10m for minutes, 2h for hours, etc.). Set 0 for permanent.')
                .setRequired(true))
        .addStringOption(option =>
            option
                .setName('reason')
    ...
    async execute(interaction) {
        ...
        if (!member.moderatable) {
            return interaction.reply({ content: 'I cannot ban this member. Check my permissions and role hierarchy.', ephemeral: true });
        }
        ...
        switch (unit) {
            ...
            default: durationMs = 0; break; // Permanent ban
        }

        if (durationMs === 0 && unit !== '') {
            return interaction.reply({ content: 'Invalid duration unit. Use s, m, h, d, w, or y.', ephemeral: true });
        }
        ...
        try {
            ...
            if (dmMember) {
                const dmEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: 'Duration', value: unit ? `${value}${unit}` : 'Permanent', inline: true },
            ...
            if (durationMs > 0) {
                setTimeout(async () => {
                    try {
                        await interaction.guild.members.unban(member.user.id, 'Ban duration expired.');
