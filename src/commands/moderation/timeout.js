    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("duration").setDescription("Timeout duration (e.g., 10m for minutes, 2h for hours, etc.)").setRequired(true)
        )
        .addStringOption((option) =>
          option.setName("reason").setDescription("Reason for the timeout.").setRequired(false)
        )
        .addBooleanOption((option) =>
          option.setName("dm_member").setDescription("DM the member?").setRequired(false)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((option) =>
          option.setName("member").setDescription("The member to remove the timeout from").setRequired(true)
        )
    ),
  ...
  run: async ({ interaction }) => {
    ...
    if (subcommand === "add") {
      ...
      if (!member.moderatable) {
        return interaction.reply({
          content: "Cannot timeout this member, check my permissions.",
          ephemeral: true,
        });
      }
      ...
      switch (unit) {
        ...
        default:
          durationMs = 0;
          break; // Shouldn't happen as regex ensures format
      }

      if (durationMs <= 0) {
        return interaction.reply({
          content: "Invalid duration. Use a positive number with valid units.",
          ephemeral: true,
        });
      }
    ...
    if (subcommand === "remove") {
      ...
      if (!member) {
        return interaction.reply({
          content: "The specified member could not be found.",
          ephemeral: true,
        });
      }
